...
public class DoctorDashboardController extends DoctorController implements Initializable {
	...
	@FXML
	private PatientTabViewController patientTabViewController;
	@FXML
	private Label labelUpdateNotesDoc;
	@FXML
	private Label labelTherapyDoc;
	...
	public void setEnviroment(Patient selectedPatient, Doctor currentDoc) throws SQLException {
		...
		int idDoc = patient.getLastModifiedBy();

		if (idDoc > 0) {
			labelUpdateNotesDoc.setText(clinic.findDoctorById(idDoc).getUsername());
		} else {
			labelUpdateNotesDoc.setText("nessuno ha ancora effettuato una modifica");
		}
		...
		if (patientTabViewController != null && doctor != null) {
			...
			patientTabViewController.setFilteredPatient(FXCollections.observableArrayList(doctor.getPatients()));
			...
			alertService.checkHighMeasurements(selectedPatient);
	...
	private void loadSymptoms() {
		try {
			ObservableList<Symptoms> symptoms = FXCollections
					.observableArrayList(clinic.loadOpenSymptoms(patient.getPatientId()));
	...
	private void loadMeasurements() {
		try {
			ObservableList<Measurement> measurments = FXCollections
					.observableArrayList(clinic.loadMeasurements(patient.getPatientId()));
	...
	public void updateNotes(ActionEvent event) {
		...
		if (confirmation) {
			int rows = 0;
			try {
				rows = clinic.updateInformation(patient.getPatientId(), newText, doctor.getMedicoId());
				labelUpdateNotesDoc.setText(doctor.getUsername());
			} catch (SQLException e) {
				e.printStackTrace();
			}
			if (rows > 0)
	...
	public void insertTherapy(ActionEvent event) {
		...
		if (medicineField.getText() == null || medicineField.getText().isBlank() || amount.getText() == null
				|| amount.getText().isBlank() || measurementUnitDropList.getValue() == null
		...
		Prescription p = new Prescription(0, doses, unit, qty, indications, patient.getPatientId(),
				doctor.getMedicoId(), drug, "No", doctor.getUsername());
		try {
	...
	public void modifyTherapy(ActionEvent e) throws IOException {
		...
		if (pSelected != null) {
			UpdatePrescriptionController controller = ViewNavigator.loadViewOver("updatePrescriptionView.fxml",
					"Update");

			
			controller.setUser(this.doctor); // erediti da DoctorController
